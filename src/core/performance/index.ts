...
export function useResourceMonitor(options: ResourceMonitorOptions = {}) {
  const { thresholds = {}, debug = false, collectionInterval = 5000 } = options;
  ...
  const logDebug = (message: string, data?: unknown) => {
    ...
    logger.log(`[Vibing Performance Monitor] ${message}`, data || '');
  };
  ...
  const collectMetrics = () => {
    ...
    if (memoryUsed > effectiveThresholds.memory.critical) {
      logger.warn(`Critical memory usage: ${(memoryUsed / 1024 / 1024).toFixed(2)}MB`);
    } else if (memoryUsed > effectiveThresholds.memory.warning) {
      logger.warn(`High memory usage: ${(memoryUsed / 1024 / 1024).toFixed(2)}MB`);
    }

    if (averageRenderTime > effectiveThresholds.renderTime.critical) {
      logger.warn(`Critical render time: ${averageRenderTime.toFixed(2)}ms`);
    } else if (averageRenderTime > effectiveThresholds.renderTime.warning) {
      logger.warn(`Slow render time: ${averageRenderTime.toFixed(2)}ms`);
    }

    if (operationCountRef.current > effectiveThresholds.operationCount.critical) {
      logger.warn(`Critical operation count: ${operationCountRef.current}`);
    } else if (operationCountRef.current > effectiveThresholds.operationCount.warning) {
      logger.warn(`High operation count: ${operationCountRef.current}`);
    }
  ...
  const measureExecution = async <T>(fn: () => Promise<T> | T, label?: string): Promise<T> => {
    ...
    try {
      ...
      logDebug(`Execution ${label ? `"${label}" ` : ''}completed in ${executionTime.toFixed(2)}ms`);

      // Warning if execution time exceeds thresholds
      if (executionTime > effectiveThresholds.executionTime.critical) {
        logger.warn(
          `Critical execution time${label ? ` for "${label}"` : ''}: ${executionTime.toFixed(2)}ms`
        );
      } else if (executionTime > effectiveThresholds.executionTime.warning) {
        logger.warn(
          `Slow execution time${label ? ` for "${label}"` : ''}: ${executionTime.toFixed(2)}ms`
        );
      }
      ...
      const executionTime = performance.now() - start;
      logDebug(
        `Execution ${label ? `"${label}" ` : ''}failed after ${executionTime.toFixed(2)}ms`,
        error
      );
      throw error;
    }
  ...
  useEffect(() => {
    ...
    if (renderTime > effectiveThresholds.renderTime.critical) {
      logger.warn(`Critical render time: ${renderTime.toFixed(2)}ms`);
    } else if (renderTime > effectiveThresholds.renderTime.warning) {
      logger.warn(`Slow render time: ${renderTime.toFixed(2)}ms`);
    }
  });
...
export function measureFunction<T>(fn: () => T, label?: string): { result: T; time: number } {
  ...
  try {
    ...
    if (label) {
      logger.log(`[Vibing Performance] ${label}: ${time.toFixed(2)}ms`);
    }
    ...
    if (label) {
      logger.error(`[Vibing Performance] ${label} failed after ${time.toFixed(2)}ms`, error);
    }
...
export function createRenderTracker(
  ...
  return {
    start: () => {
      ...
      if (debug) {
        logger.log(`[Vibing Performance] ${componentName} rendered in ${renderTime.toFixed(2)}ms`);
      }

      if (renderTime > warnThreshold) {
        logger.warn(
          `[Vibing Performance] Slow render detected: ${componentName} took ${renderTime.toFixed(2)}ms`
        );
      }
...
export function recordMemoryUsage() {
  ...
  if (!memory) {
    logger.warn('[Vibing Performance] Performance.memory API not available in this browser');
    return {
      used: 0,
      total: 0,
      limit: 0,
