...
export function useSuperAgent() {
   ...
   */
  const askSuperAgent = useCallback(async (
    query: string,
    options: QueryOptions = {}
  ): Promise<AgentResponse> => {
    console.log('[Super Agent] Query:', query, 'Options:', options);
      ...
      suggestedActions: [
        {
          ...
          icon: 'bolt',
          action: () => console.log('Example action executed')
        }
      ]
    };
  }, []);
  ...
  const suggestAction = useCallback((suggestion: ActionSuggestion) => {
    console.log('[Super Agent] Action suggestion:', suggestion);
  ...
  const onIntent = useCallback((
    ...
    console.log(`[Super Agent] Registered handler for intent: ${intent}`);
  ...
  const handleIntent = useCallback(async (
    ...
    console.log(`[Super Agent] Handling intent: ${intent}`, parameters);
